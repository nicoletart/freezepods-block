...
export default function MagnetometerPage({ gameType }) {
  ...
  const startNotifications = async (server, gameType) => {
    try {
      ...
      if (gameType === "magnetometer") {
        const magnetometerService = await server.getPrimaryService(
          MicrobitUuid.magnetometerService[0]
        );
        if (!magnetometerService) {
          console.error("Magnetometer Service not found");
          return;
        }
        console.log("Magnetometer Service:", magnetometerService);
        const magnetometerDataCharacteristic =
          await magnetometerService.getCharacteristic(
            MicrobitUuid.magnetometerData[0]
          );
        console.log(
  ...
  const handleMagnetometerDataChanged = (event) => {
    ...
    let normalizedHeading = headingRadians * (180 / Math.PI);

    if (normalizedHeading < 0) {
      normalizedHeading += 360;
    }

    console.log(`Raw X: ${x}, Y: ${y}, Z: ${z}`);
    console.log(`Calculated Heading: ${normalizedHeading}°`);

    if (lastHeading !== null) {
      const headingDifference = Math.abs(normalizedHeading - lastHeading);

      if (
        headingDifference > 170 &&
        headingDifference < 190 &&
        localRound.current === gameState.round
      ) {
        gameState.round = gameState.round + 1;
        setGameState((prev) => ({
        ...
        console.log("Direction changed by about 180 degrees");
        nextRound();
      }
    }
  };
      ...
      {gameOver ? (
        ...
        <>
          ...
          <div className="hit-target">
            <p>
              Move the Micro:bit near a magnet to score points!
            </p>
          </div>
        </>
      )}
